...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes=num_classes):
        ...
        # Entrada: [B, 3, 64, 64]  →  Salida: [B, 16, 32, 32]
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(16)

        # 🔹 Capa 2: Conv -> BatchNorm -> ReLU -> Pool
        # Entrada: [B, 16, 32, 32]  →  Salida: [B, 32, 16, 16]
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(32)

        # 🔹 Capa 3: Conv -> BatchNorm -> ReLU -> Pool
        # Entrada: [B, 32, 16, 16]  →  Salida: [B, 64, 8, 8]
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, padding=1)
        self.bn3 = nn.BatchNorm2d(64)

        # 🔹 Capa totalmente conectada (clasificación)
        # Aplanamos: [B, 64, 8, 8] → [B, 64*8*8 = 4096]
        self.fc1 = nn.Linear(64 * 8 * 8, 256)
        self.fc2 = nn.Linear(256, num_classes)

    def forward(self, x):
        # ---- Capa 1 ----
        x = self.conv1(x)              # [B, 3, 64, 64] → [B, 16, 64, 64]
        x = self.bn1(x)
        x = self.relu(x)
        x = self.pool(x)               # [B, 16, 32, 32]  (↓ tamaño a la mitad)

        # ---- Capa 2 ----
        x = self.conv2(x)              # [B, 16, 32, 32] → [B, 32, 32, 32]
        x = self.bn2(x)
        x = self.relu(x)
        x = self.pool(x)               # [B, 32, 16, 16]

        # ---- Capa 3 ----
        x = self.conv3(x)              # [B, 32, 16, 16] → [B, 64, 16, 16]
        x = self.bn3(x)
        x = self.relu(x)
        x = self.pool(x)               # [B, 64, 8, 8]

        # ---- Clasificador ----
        x = torch.flatten(x, 1)        # [B, 64, 8, 8] → [B, 4096]
